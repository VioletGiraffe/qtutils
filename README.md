...
| `widgets/cfindbar.h` | Find bar searching through host-supplied functions: pattern history, previous and next, case, whole-word and regex toggles, a status for misses and wrap-arounds, optional `QSettings` persistence; its find actions carry host-chosen keys. |
| `widgets/findresult.h` | Search outcome returned by `CLightningFastViewerWidget::find` and by `CFindBar`'s host functions: not found, found, or found after wrapping around. |
